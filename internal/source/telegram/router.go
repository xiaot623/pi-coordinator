...
func (r *Router) handleCommand(ctx context.Context, update Update) {
	...
	if handler, ok := r.commandHandlers[cmd]; ok {
		...
		r.bot.send(msg.Chat.ID, "Unknown command. Available: /help /workspace /add /new /todo /status /detail /stop /open /rebase /commit /diff /sync /pin /unpin /model /bots /newbot", nil)
	}
}
